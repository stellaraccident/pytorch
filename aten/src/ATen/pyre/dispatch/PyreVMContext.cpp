...
CachedKernel loadKernel(
  ...
  // Resolve entry point. Torch-mlir convention uses $async suffix;
  // native IREE convention uses the bare function name.
  std::string full_name = native_abi
      ? "module." + func_name
      : "module." + func_name + "$async";
